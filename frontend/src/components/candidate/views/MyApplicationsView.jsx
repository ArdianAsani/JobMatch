...
const MyApplicationsView = () => {
  ...
  const handleCancel = async (appId) => {
    ...
    try {
      ...
      alert(err.response?.data?.detail || 'Failed to retract.')
    }
  }
    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {filtered.map(app => {
            const matchColor = app.match_score >= 90 ? 'bg-green-50 text-green-700'
              : app.match_score >= 80 ? 'bg-teal-50 text-teal-700'
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${matchColor}`}>
                          <Zap size={9}/>{app.match_score}% Match
                        </span>
